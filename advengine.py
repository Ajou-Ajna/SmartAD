...
# 장면 전환(카메라 컷) 감지 민감도 (scedet 기준, 기본 10.0)
SCENE_THRESHOLD = 7.0
...
def detect_scene_changes(video_path, threshold=10.0, start_time=None, end_time=None, min_scene_duration=3.0):
    """FFmpeg scdet + metadata=mode=print:file=- 를 이용해 장면 전환 메타데이터를 안정적으로 추출합니다."""
    ...
    cmd.extend([
        "-i", video_path,
        "-filter:v", f"scedet=threshold={threshold},metadata=mode=print:file=-",
        "-an",
        "-f", "null", "-"
    ])
    ...
    # metadata=mode=print:file=- 는 stdout으로 나오지만,
    # FFmpeg 버전에 따라 일부 정보가 stderr에 섞일 수 있으므로 둘 다 합쳐서 파싱한다.
    ...
    for line in combined_output.splitlines():
        score_match = re.search(r"lavfi\.scd\.score=([0-9.]+)", line)
        if score_match:
            pending_score = float(score_match.group(1))

        time_match = re.search(r"lavfi\.scd\.time=([0-9.]+)", line)
        if time_match:
            pending_time = float(time_match.group(1))

        if pending_time is not None:
            if pending_time > 0.1:  # 극초반 0초에 잡히는 가짜 컷은 무시
                raw_scene_events.append({
                    'time': pending_time,
                    'score': pending_score if pending_score is not None else 0.0,
                })
            pending_time = None
            pending_score = None

    # 장면전환 감지 이후 일정 시간(min_scene_duration) 동안 추가 감지를 막는 쿨다운 필터링
    filtered_scene_events = []
    last_kept_time = None
    for event in raw_scene_events:
        scene_time = event['time']
        if last_kept_time is None or scene_time >= last_kept_time + min_scene_duration:
            filtered_scene_events.append(event)
            last_kept_time = scene_time

    return filtered_scene_events
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        if not silence_summaries:
            ...
            for summary in silence_summaries:
                ...
                if not summary['scene_events']:
                    f.write("장면전환 NULL\n")
                else:
                    for scene_idx, scene_event in enumerate(summary['scene_events'], start=1):
                        absolute_scene_time = summary['start'] + scene_event['time']
...
def main():
    ...
    for silence in silence_timestamps:
        ...
        if scene_events:
            print(f"   -> {len(scene_events)}번의 장면 전환 발견. 프레임 이미지 추출 및 음원 추출 시작...")

            # 컷 전/후 프레임 이미지 추출 (직전 0.1초, 직후 사용자 설정 초 이후 프레임 캡처)
            for idx, scene_event in enumerate(scene_events):
                absolute_cut_time = start + scene_event['time']
                before_time = max(0.0, absolute_cut_time - 0.1)
                after_time = absolute_cut_time + SCENE_GAP
                scene_score = scene_event['score']
                ...
                print(
                    f"      -> 컷 {idx + 1:02d}: 절대시각 {absolute_cut_time:.2f}초, "
                    f"score={scene_score:.2f}, 캡처 완료"
                )
        else:
            print(f"   -> 장면 전환 없음.")
